...
class TestResult(models.Document):
    ...
    response_time = models.FloatProperty("Time in which a response/error was returned") 
    run_details = models.ListProperty("Additional info about the test execution")
    """ 
    Optional Fields:
        url = models.StringProperty("URL on which the test run")
        msg = models.StringProperty("Brief description") # might be not available
        response_time_min = models.FloatProperty("Min time of response in case of multiple runs") 
        response_time_max = models.FloatProperty("Max time of response in case of multiple runs")
        response_time_std = models.FloatProperty("Standard deviation of time of response values in case of multiple runs")
    """